...
type StatusConsultoria = 'em_andamento' | 'concluida' | 'aguardando' | 'cancelada';
...
const STATUS_CONFIG: Record<StatusConsultoria, { label: string; color: string }> = {
...
const CONSULTORIAS_INICIAIS: Consultoria[] = [
  { id: '1', titulo: 'Parecer sobre cláusula contratual', cliente: 'Empresa Alfa Ltda', valorEstimado: 3500, status: 'concluida', data: '2026-08-15' },
  { id: '2', titulo: 'Due diligence — fusão societária', cliente: 'Beta Comércio SA', valorEstimado: 12000, status: 'em_andamento', data: '2026-09-01' },
  { id: '3', titulo: 'Orientação tributária — reestruturação', cliente: 'João da Silva', valorEstimado: 5000, status: 'aguardando', data: '2026-09-10' },
  { id: '4', titulo: 'Análise de contrato de locação', cliente: 'Maria Oliveira', valorEstimado: 1800, status: 'concluida', data: '2026-07-20' },
];
...
function NovaConsultoriaModal({ onClose, onSave }: {
  onClose: () => void;
  onSave: (c: Omit<Consultoria, 'id'>) => void;
}) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSave({
      ...
      valorEstimado: parseFloat(valor.replace(',', '.')) || 0,
      status,
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as StatusConsultoria)}
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2d5e]/30"
            >
              {(Object.keys(STATUS_CONFIG) as StatusConsultoria[]).map((s) => (
...
export default function ConsultoriasPage() {
  const [consultorias, setConsultorias] = useState<Consultoria[]>(CONSULTORIAS_INICIAIS);
  const [showModal, setShowModal] = useState(false);
  const [busca, setBusca] = useState('');

  function addConsultoria(data: Omit<Consultoria, 'id'>) {
    setConsultorias((prev) => [{ ...data, id: String(Date.now()) }, ...prev]);
  }

  const filtered = consultorias.filter(
    (c) =>
      c.titulo.toLowerCase().includes(busca.toLowerCase()) ||
      c.cliente.toLowerCase().includes(busca.toLowerCase())
  );

  const totalEstimado = filtered.reduce((sum, c) => sum + c.valorEstimado, 0);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-[#6b7280]">
            {filtered.length} consultorias · Total estimado: {formatBRL(totalEstimado)}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2.5 border border-[#e5e7eb] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2d5e]/30"
        />
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((c) => (
                <tr key={c.id} className="border-b border-[#f1f5f9] hover:bg-[#f8fafc] transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-[#6b7280]">{c.cliente}</td>
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex px-2.5 py-0.5 text-[11px] font-semibold rounded-full ${STATUS_CONFIG[c.status].color}`}>
